            ...
            <b-container class="py-0">
                <b-row>
                    ...
                    <b-col class="d-flex align-items-center" cols="4" md="4" sm="4">
                        ...
                        <h1><router-link :to="{name: 'Home'}">Kenaz</router-link></h1>
                    </b-col>
...
export default {
    ...
    methods: {
        ...
        onSubmit() {
            if(this.queryString) {
                this.$router.push({ name: 'Category', params: {name: this.link.name ,link: this.link.link}, query: {queryString: this.queryString}})
                this.queryString = ''
    ...
    computed: {
        link() {
            return this.links[+this.category]
        }
    }
}
</script>
